import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { listPersonnel, deletePersonnel } from '../../api/personnel';
import { listRooms } from '../../api/rooms';
import '../../styles/crud.css';

function PersonnelListPage() {
  const navigate = useNavigate();
  const [personnel, setPersonnel] = useState([]);
  const [rooms, setRooms] = useState([]);
  const [roomCode, setRoomCode] = useState('');
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;

    async function load() {
      setLoading(true);
      setError(null);
      try {
        const [personnelList, roomList] = await Promise.all([
          listPersonnel({ roomCode }),
          listRooms(),
        ]);
        if (!cancelled) {
          setPersonnel(personnelList);
          setRooms(roomList);
        }
      } catch (err) {
        if (!cancelled) setError(err.message);
      } finally {
        if (!cancelled) setLoading(false);
      }
    }

    load();

    return () => {
      cancelled = true;
    };
  }, [roomCode]);

  async function handleDelete(event, serviceNumber) {
    event.stopPropagation();
    if (!window.confirm(`${serviceNumber} 인원을 삭제하시겠습니까?`)) return;

    try {
      await deletePersonnel(serviceNumber);
      setPersonnel((prev) => prev.filter((p) => p.service_number !== serviceNumber));
    } catch (err) {
      setError(err.message);
    }
  }

  return (
    <div>
      <h2>사용자 등록</h2>

      <div className="page-toolbar">
        <select value={roomCode} onChange={(e) => setRoomCode(e.target.value)}>
          <option value="">전체 내무반</option>
          {rooms.map((room) => (
            <option key={room.room_code} value={room.room_code}>
              {room.room_code} — {room.room_name || '(이름 없음)'}
            </option>
          ))}
        </select>
        <div className="spacer" />
        <button type="button" onClick={() => navigate('/personnel/match')}>
          RFID 매칭
        </button>
        <button type="button" className="primary" onClick={() => navigate('/personnel/new')}>
          + 새 인원
        </button>
      </div>

      {error && <div className="banner-error">{error}</div>}

      {loading ? (
        <p>불러오는 중...</p>
      ) : (
        <table className="data-table">
          <thead>
            <tr>
              <th>군번</th>
              <th>이름</th>
              <th>전화번호</th>
              <th>내무반</th>
              <th>RFID 매칭</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {personnel.length === 0 && (
              <tr>
                <td colSpan={6}>등록된 인원이 없습니다.</td>
              </tr>
            )}
            {personnel.map((person) => (
              <tr
                key={person.service_number}
                onClick={() => navigate(`/personnel/${person.service_number}/edit`)}
              >
                <td>{person.service_number}</td>
                <td>{person.name}</td>
                <td>{person.phone_number || '-'}</td>
                <td>{person.room_code}</td>
                <td>
                  <span className={`badge ${person.rfid_uid ? 'badge-online' : 'badge-offline'}`}>
                    {person.rfid_uid ? '매칭됨' : '미매칭'}
                  </span>
                </td>
                <td className="actions">
                  <button type="button" onClick={(e) => handleDelete(e, person.service_number)}>
                    삭제
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      )}
    </div>
  );
}

export default PersonnelListPage;
